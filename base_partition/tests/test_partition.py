...
class TestPartition(TransactionCase):
    ...
    def test_partition_dotdot_relational(self):
        self.partition_field_test(
            "parent_id.category_id", relational=True, dotdot=True
        )

    def test_partition_dotdot_nonrelational(self):
        self.partition_field_test(
            "parent_id.name", relational=False, dotdot=True
        )
    ...
    def test_batch(self):
        ...
        assert records  # only makes sense with nonempty recordset
        batches = list(records.batch(batch_size))
        self.assertEqual(
            len(batches), math.ceil(1.0 * len(records) / batch_size)
        )
    ...
    def test_filtered_domain(self):
        ...
        self.assertEqual(
            empty_recordset, records.filtered_domain([(0, "=", 1)])
        )
