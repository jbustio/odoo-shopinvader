...
class WishlistLineCreateRequest(StrictExtendableBaseModel):
...
class WishlistCreateRequest(StrictExtendableBaseModel):
    name: str
...
class WishlistUpdateRequest(StrictExtendableBaseModel):
    name: str | None = None
    lines: list[WishlistLineCreateRequest] | None = None
    ref: str | None = None
    typology: str = None
...
class WishlistDeleteItemRequest(WishlistLineIdentifier):
    ...


class WishlistdMoveItemRequest(WishlistLineIdentifier):
    move_to_wishlist_id: int


class WishlistAddItemRequest(WishlistLineIdentifier):
    product_id: int
    quantity: float | None = 1.0
    sequence: int | None = None
...
class WishlistUpdateItemRequest(WishlistAddItemRequest):
    ...


class WishlistReplaceItemRequest(WishlistLineIdentifier):
    product_id: int
    replacement_product_id: int


class WishlistSearchRequest(StrictExtendableBaseModel):
    id: int | None = None
    name: Annotated[
        str | None,
        Field(
...
class WishlistItemToCartRequest(StrictExtendableBaseModel):
    product_id: int


class WishlistAddItemsToCartRequest(StrictExtendableBaseModel):
    uuid: str | None = None
    lines: list[WishlistItemToCartRequest] = []


class WishlistAddToCartRequest(StrictExtendableBaseModel):
    uuid: str | None = None
