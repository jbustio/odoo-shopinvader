...
class ShopinvaderVariant(models.Model):
    ...
    def _action_product_toggle_active(self):
        ...
        yield
        shop_products_to_active = self.env["shopinvader.product"].browse()
        shop_products_to_inactive = self.env["shopinvader.product"].browse()
        for variant in self:
            ...
            if variant.active:
                ...
                if not shopinv_product.active:
                    shop_products_to_active |= shopinv_product
                continue
            ...
            if all(
                ...
                shop_products_to_inactive |= shopinv_product
        if shop_products_to_active:
            shop_products_to_active.write({"active": True})
        if shop_products_to_inactive:
            shop_products_to_inactive.write({"active": False})
