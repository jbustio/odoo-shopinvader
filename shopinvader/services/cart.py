...
class CartService(Component):
    ...
    def _cart_validator_exists(self, field, value, error):
        cart = self.env["sale.order"].search(
            self._get_cart_by_id_domain(value)
        )
        if len(cart) != 1:
            error(field, "The cart does not exists!")
