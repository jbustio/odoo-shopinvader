...
class AbstractItemCase(object):
    ...
    def test_add_unbinded_item_with_an_existing_cart(self):
        self.sale_line_obj = self.env["sale.order.line"]
        self.unbinded_product = self.product_3.copy()
        cart = self.service.search()["data"]
        nbr_line = len(cart["lines"]["items"])
        vals = {"product_id": self.unbinded_product.id}
        values = self.sale_line_obj.play_onchanges(vals)
        values.update(
            {
                "order_id": cart.get("id"),
                "product_uom_qty": 3.0,
                "product_id": self.unbinded_product.id,
            }
        )
        ...
        self.shopinvader_backend.authorize_not_binded_products = True
        cart = self.service.search()["data"]
        product_json = cart["lines"]["items"][-1]["product"]
        self.assertEquals(
            product_json, {"id": 121, "name": u"iPod", "sku": u"E-COM12"}
        )
