...
class CartClearTest(object):
    def _check_clear_cart_result(self, cart):
        ...
        elif clear_option in ["new", "new_cancel"]:
            self.assertEquals(len(new_carts), 1)
            self.assertIsInstance(session, dict)
            self.assertEquals(session.get("cart_id"), new_carts.id)
            # The previous should exists
            self.assertEquals(cart_id, cart.exists().id)
            if clear_option == "new_cancel":
    ...
    def test_cart_new(self):
        self.backend.write({"clear_cart_options": "new"})
        self._check_clear_cart_result(self.cart)

    def test_cart_new_cancel(self):
        self.backend.write({"clear_cart_options": "new_cancel"})
        self._check_clear_cart_result(self.cart)
