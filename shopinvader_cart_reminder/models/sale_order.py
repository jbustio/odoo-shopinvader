...
class SaleOrder(models.Model):
    ...
    def _get_sale_order_for_reminder(self, backend, reminder_date):
        ...
        """
        anonymous_user = backend.mapped("anonymous_partner_id")
        domain = [
            ("partner_id", "!=", anonymous_user.id),
    ...
    def _execute_reminder(self):
        ...
        for sale_order in self:
            mail_template = (
                sale_order.shopinvader_backend_id.quotation_reminder_mail_template_id
            )
            mail_template.send_mail(sale_order.id)
        self.write({"last_reminder": fields.Datetime.now()})
    ...
    def _get_reminder_date(self, backend):
        ...
        reminder_date -= timedelta(hours=backend.quotation_reminder)
        reminder_date = fields.Datetime.context_timestamp(self, reminder_date)
