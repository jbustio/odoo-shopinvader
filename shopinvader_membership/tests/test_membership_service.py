...
class TestMembershipService(CommonCase):
    ...
    def setUp(self, *args, **kwargs):
        ...
        self.membership_line_obj = self.env["membership.membership_line"]
        self.product_obj = self.env["product.product"]
        self.partner = self.env.ref("shopinvader.partner_1")
        str_today = fields.Date.today()
        self.date_today = fields.Date.from_string(str_today)
        self.next_month = fields.Date.to_string(
            self.date_today + timedelta(days=30)
        )
        self.product = self.product_obj.create({
            'type': 'service',
            'name': 'Membership Test Product',
            'membership': True,
            'membership_date_from': self.date_today,
    ...
    def _check_data_content(self, data, membership_lines):
        ...
        for current_data, membership_line in zip(data, membership_lines):
            ...
            self.assertEquals(
                current_data.get("membership_line_id"),
                membership_line.id
            )
            self.assertEquals(
                current_data.get("date") or False,
                membership_line.date
            )
            self.assertEquals(
            ...
            self.assertEquals(
                current_data.get("date_to") or False,
